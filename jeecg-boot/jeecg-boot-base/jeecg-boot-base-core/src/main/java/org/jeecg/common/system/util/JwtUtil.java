...
//	}
	...
	/**
	 * @Author fuwang
	 * @Description 利用用户id和人员类型前缀生成签名  有效期为5分钟
	 * @param
	 * @Return
	 * @Date 2021/12/7 14:00
	 */
	public static String newsign(String userId, String userType) {
		Date date = new Date(System.currentTimeMillis() + EXPIRE_TIME);
		Algorithm algorithm = Algorithm.HMAC256(userId);
		// 附带username信息
		return JWT.create().withClaim("userId", userId).withClaim("userType",userType).withExpiresAt(date).sign(algorithm);
	...
	public static boolean newverify(String token, String userId, String userType) {
		try {
			// 根据密码生成JWT效验器
			Algorithm algorithm = Algorithm.HMAC256(userId);
