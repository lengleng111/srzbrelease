...
//			}
	...
	@Override
	public SrEmployeeEntity getsrygById(String userId) {
		if(oConvertUtils.isEmpty(userId)) {
			return null;
		}
		org.jeecg.modules.srzb.employee.dao.entity.SrEmployeeEntity srEmployeeEntity=srEmployeeMapper.selectById(userId);
		SrEmployeeEntity srEmployeeEntity1=new SrEmployeeEntity();
		BeanUtils.copyProperties(srEmployeeEntity,srEmployeeEntity1);
		if(srEmployeeEntity==null) {
			return null;
		}
		return srEmployeeEntity1;
	}
	
	@Cacheable(cacheNames=CacheConstant.SYS_USERS_CACHE, key="#userId")
	@Override
	public SrUserEntity getsrgkById(String userId) {
		if(oConvertUtils.isEmpty(userId)) {
			return null;
		}
		org.jeecg.modules.srzb.user.dao.entity.SrUserEntity srUserEntity=srUserMapper.selectById(userId);
		SrUserEntity srUserEntity1=new SrUserEntity();
		BeanUtils.copyProperties(srUserEntity,srUserEntity1);
		if(srUserEntity==null) {
			return null;
		}
		return srUserEntity1;
	}
